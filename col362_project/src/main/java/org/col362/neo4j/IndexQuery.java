...
public class IndexQuery {
    ...
    @Procedure(name = "org.col362.neo4j.indexQuery", mode = Mode.WRITE)
    ...
    public Stream<booleanresult> reachabilityQuery(@Name("node_from_id") String nf_id, @Name("node_to_id") String nt_id, @Name("time") String time) {
        ...
        try {
            ...
            postingsList pl = new postingsList(Global.N,Global.T,"/Users/bhavukb/Desktop/Courses/Sem8/DBMS/project/col362_project/indices/index");
            pl.offset1 = Global.offset1;
            ArrayList<pair> alf = pl.readPostingsList(nf-1);
            ArrayList<pair> alt = pl.readPostingsList(nt-1);
            pair p = new pair(t, Integer.MIN_VALUE);
            int indf = Collections.binarySearch(alf, p, new pairComparator());
            int indt = Collections.binarySearch(alt, p, new pairComparator());
            indf = -indf - 1;
            indt = -indt - 1;
            int sccf = alf.get(indf).second;
            int scct = alt.get(indt).second;
            ...
            Node nscc1 = tx.findNode(Label.label("SCC"), "scc_id", sccf);
            Node nscc2 = tx.findNode(Label.label("SCC"), "scc_id", scct);
            ...
            br = new booleanresult(dfs(nscc1, nscc2, t));
            return Stream.of(br);
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean dfs(Node u, Node v, int time) {
        if (u.equals(v))
            return true;
        visited[(int)u.getProperty("scc_id")-1] = 1;
        ResourceIterator<Relationship> ri = u.getRelationships(Direction.OUTGOING).iterator();
        while(ri.hasNext()) {
            Relationship r = ri.next();
            Node n = r.getEndNode();
            if (entityExists(n,time) && entityExists(r, time) && visited[(int)n.getProperty("scc_id")-1] == 0) {
                if (dfs(n, v, time))
                    return true;
            }
        }
