...
public class createIndexForFile
{
    ...
    public void createGraph(@Name("numNodes") String num_nodes, @Name("numTimes") String times, @Name("filePath") String filepath, @Name("delimiter") String delimiter)
    {
        ...
        T = Global.T;
        initialize(txn);
        // initializer1(txn, filepath, delimiter);
        // initializer2(txn, filepath, delimiter);
        txn.commit();
        try
        {
            txn = db.beginTx();
            ...
            while ((records = br.readLine()) != null)
            {
                ...
                // Call the updater
                processUpdate(n1, n2, updater);

                // Update the postings list
                updatePostingList(txn);

                // Create the SCC node if it does not exist
                createSCCNode(txn, n1, n2);
    ...
    private void processUpdate(Node n1, Node n2, sccUpdater updater)
    {
    ...
    private void updatePostingList(Transaction txn)
    {
        ...
        while(nodes.hasNext()) {
            ...
            pair p = new pair(scc, time);
            //binary search the location of pair p in al 
            int index = Collections.binarySearch(al, p, new pairComparator());
            if(index < 0) {
                index = -index - 1;
            }
            al.set(index, p);
